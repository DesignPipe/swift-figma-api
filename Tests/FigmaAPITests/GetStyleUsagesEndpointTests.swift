...
final class GetStyleUsagesEndpointTests: XCTestCase {
    // MARK: - URL Construction

    func testMakeRequestConstructsCorrectURL() throws {
        let endpoint = GetStyleUsagesEndpoint(fileKey: "file123")
        ...
        XCTAssertEqual(
            ...
            "https://api.figma.com/v1/analytics/libraries/file123/style/usages"
        )
    }
    ...
    func testContentParsesLibraryUsagesResponse() throws {
        ...
        let endpoint = GetStyleUsagesEndpoint(fileKey: "test")
        let usages = endpoint.content(from: response)

        XCTAssertEqual(usages.count, 1)
        XCTAssertEqual(usages[0].usageCount, 150)
    }
